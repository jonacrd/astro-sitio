-- Script para eliminar funciones duplicadas de place_order y crear una única versión

-- 1. Eliminar TODAS las versiones de place_order
DROP FUNCTION IF EXISTS place_order(UUID, UUID, VARCHAR);
DROP FUNCTION IF EXISTS place_order(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS place_order(UUID, UUID, TEXT, JSONB, TEXT);

-- 2. Crear la versión DEFINITIVA con todos los parámetros necesarios
...
BEGIN
  ...
  -- Procesar items del carrito desde localStorage (simulado con seller_products)
  -- En este caso, asumimos que el frontend ya validó los items
  -- y los pasamos como parámetros adicionales si es necesario
  
  -- Por ahora, calcular el total desde seller_products del vendedor
  FOR v_item IN
    SELECT 
  ...
  LOOP
    ...
    SET 
      ...
      updated_at = NOW()
    WHERE id = v_item.seller_product_id;
...
-- 4. Verificar que solo existe una versión
SELECT 
