...
/**
 * Script para probar checkout con la corrección de relaciones
 * Ejecutar con: node scripts/test-checkout-fixed.js
 */
...
async function testCheckoutFixed() {
  console.log('🧪 Probando checkout con corrección de relaciones...');
  ...
  try {
    ...
    // 6. Verificar carrito con la relación corregida
    const { data: cartWithSeller, error: cartWithSellerError } = await supabaseAdmin
      .from('carts')
      .select(`
        id,
        seller_id,
    ...
    if (cartWithSellerError) {
      console.error('❌ Error obteniendo carrito con vendedor:', cartWithSellerError);
      return;
    }

    console.log('✅ Carrito con vendedor:', cartWithSeller.seller.name);
    ...
    // 8. Probar checkout con autenticación real
