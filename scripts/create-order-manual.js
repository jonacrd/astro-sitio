...
/**
 * Script para crear una orden manualmente en la base de datos
 * Ejecutar con: node scripts/create-order-manual.js
 */
...
async function createOrderManual() {
  console.log('🛒 Creando orden manualmente...');
  ...
  try {
    ...
    if (deleteCartError) {
      console.error('❌ Error limpiando carrito:', deleteCartError);
      return;
    }

    const { error: deleteCartError2 } = await supabase
      .from('carts')
      .delete()
      .eq('id', cart.id);

    if (deleteCartError2) {
      console.error('❌ Error eliminando carrito:', deleteCartError2);
      return;
    }
