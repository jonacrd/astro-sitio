...
export async function getProductsByCategory(categorySlug: string, limit: number = 8): Promise<ProductWithCategory[]> {
  return await prisma.product.findMany({
    where: {
      category: {
        slug: categorySlug
...
export async function getRandomProducts(limit: number = 8): Promise<ProductWithCategory[]> {
...
export async function getTopProductsByCategory(categorySlug: string, limit: number = 3): Promise<ProductWithCategory[]> {
  return await prisma.product.findMany({
    where: {
      category: {
        slug: categorySlug
...
export async function getBestSellingProducts(limit: number = 6): Promise<ProductWithCategory[]> {
